...
excel_file = r'C:\Python\output\temp_wms\wmsDH.xlsx'
...
def call_to_excel(data, excel_file):
    ...
    # Add timestamp to the first column
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    last_row = sheet.max_row

    # Determine the starting row for adding new data
    ...
    for item in data:
            # Append timestamp to the beginning of each row of data
            sheet.cell(row=start_row, column=1, value=timestamp)
            # Add rest of the data from the API to subsequent columns
            for col, value in enumerate(item.values(), start=2):
...
if __name__ == "__main__":
    print(f"Writing {script_name} to temp_sheet...")
    ...
    try:
        ...
        if row_count > threshold:
            call_to_excel(data_to_insert, excel_file)
            ...
            if inserted_rows != row_count:
                ...
                _DEF.log_status(connection, "Error", script_cat, script_name, start_time, _DEF.datetime.now(), int((_DEF.datetime.now() - start_time).total_seconds() / 60), row_count - inserted_rows, error_details, "Gans", api)
