...
def count_sql_rows_cl(connection, sql_table, company_name):
    cursor = connection.cursor()
    sql_query = f"SELECT COUNT(*) FROM {sql_table} WHERE [Entity] = 'Central Layer'"
    cursor.execute(sql_query, (company_name,))
    result = cursor.fetchone()
    return result[0] if result else 0
...
if __name__ == "__main__":
    ...
    for company_name in company_names:
        try:
            ...
            sql_cl_row_count = count_sql_rows_cl(connection, sql_table, company_name)
            sql_row_count = count_sql_rows(connection, sql_table, company_name)

            if sql_cl_row_count != sql_row_count:
